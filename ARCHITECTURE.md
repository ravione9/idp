...
### `0dc80b6` — 2026-09-24 — Unblock pam-2 deploy hang on stale API container remove
...
- **`scripts/compose-lib.sh`** — `idp_rm_stale_api` uses timed `docker kill` + `docker rm -f` first; compose `rm -f` without `-s` and with `timeout` (no graceful stop wait).
- **`src/index.ts`** — shutdown closes RADIUS UDP and `closeAllConnections()` on HTTP/HTTPS so `server.close` is not stalled by keep-alives.
...
**Why** — Auto-discovered Google Workspace and AD security groups stopped at 200; large tenants need every group mirrored when Sync Groups is blank or `*`.

**What changed:**

- **`src/services/group-sync.ts`** — paginate through all Google groups (no `GOOGLE_GROUP_AUTO_CAP`).
- **`src/adapters/ad-adapter.ts`** — `listDirectoryGroups()` returns all security groups from LDAP search.
- **`connectors/ad-agent/src/ad-ldap.ts`** — same for on-prem AD agent group harvest.
- **`web/js/views-stubs.js`** — connector scope hints no longer mention max 200.
...
### `328da1e` — 2026-08-10 — Fix Google portal OAuth invalid_client from mixed credentials
...
- **Router** — `attendanceIga` accepts `?tab=`; tab clicks call `syncAppUrl` (e.g. `/?v=attendanceIga&tab=policy`).
